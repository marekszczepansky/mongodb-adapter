...
MongoDB.prototype.create = function (model, data, callback, modelConstructor, options) {
    ...
    this.collection(model).insertOne(data, options, function (err, m) {
        if (err || (options.w < 1)) {
            callback(err, null);
        } else {
            callback(err, m.insertedId);
        }
    });
};
...
MongoDB.prototype.save = function (model, data, callback) {
    ...
    this.collection(model).updateOne({_id: id},  {$set: data}, function (err) {
        callback(err);
    });
};
...
MongoDB.prototype.updateOrCreate = function updateOrCreate(model, data, callback) {
    ...
    if (typeof id === "string") {
        ...
        id = data._id ? data._id : new bson.ObjectID();
    }
    ...
    this.collection(model).updateOne({_id: id}, {$set: data}, {upsert: true, multi: false, strict: false}, function (err, rowsAffected) {
        data.id = id;
        callback(err, data);
    });
};
...
MongoDB.prototype.destroy = function destroy(model, id, callback) {
    ...
    this.collection(model).deleteOne({_id: id}, callback);
};
...
MongoDB.prototype.destroyAll = function destroyAll(model, callback) {
    this.collection(model).deleteMany({}, callback);
};

MongoDB.prototype.count = function count(model, where, callback) {
    this.collection(model).countDocuments(where || {}, function (err, count) {
        callback(err, count);
    });
};
...
MongoDB.prototype.updateAttributes = function updateAttrs(model, id, data, cb) {
    ...
    this.collection(model).findOneAndUpdate({_id: id}, {$set: data}, {}, function (err, object) {
        cb(err, object);
    });
};

MongoDB.prototype.update = function update(model, params, callback) {
    // set _id as a mongodb object
    const query = Object.assign({}, params.where);
    ...
    return this.collection(model).updateOne(query, {$set: params.update}, {multi: true}, function (err, result) {
        callback(err, result);
    });
};
