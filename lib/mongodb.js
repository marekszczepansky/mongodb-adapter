...
MongoDB.prototype.save = function (model, data, callback) {
    ...
    this.collection(model).updateOne({_id: id}, {$set: data}, function (err) {
       ...
       MongoDB.prototype.updateOrCreate = function updateOrCreate(model, data, callback) {
           ...
           this.collection(model).updateOne({_id: id}, {$set: data}, {upsert: true, multi: false}, function (err, rowsAffected) {
               data.id = id;
               callback(err, data);
           });
       };
       ...
       MongoDB.prototype.count = function count(model, callback, where) {
           this.collection(model).countDocuments(where).then(function (count, err) {
                callback(err, count);
           });
       };
